...
codeunit 82666 "RSMUS CDM Util" // Refer Common Data Model https://docs.microsoft.com/en-us/common-data-model/sdk/overview
{
    Access = Internal;
    ...
    begin
        ...
        Definition.Add('displayName', rsmusUtil.GetTableName(TableID));
        Definition.Add('description', StrSubstNo(RepresentsTableTxt, rsmusUtil.GetTableName(TableID)));
        Definition.Add('hasAttributes', CreateAttributes(TableID, FieldIdList));
        Definitions.Add(Definition);
        Content.Add('definitions', Definitions);
    end;

    procedure UpdateDefaultManifestContent(ExistingContent: JsonObject; TableID: Integer; Folder: Text; rsmusCdmFormat: Enum "RSMUS Data Export Format") Content: JsonObject
    var
        rsmusUtil: Codeunit "RSMUS Util";
        Entities: JsonArray;
    ...
    begin
        ...
        if not rsmusUtil.JsonTokenExistsWithValueInArray(Entities, 'entityName', EntityName) then begin
            ...
            case rsmusCdmFormat of
                ...
                "RSMUS Data Export Format"::JSON:
                    begin
                        DataPartitionPattern.Add('globPattern', '*.json');
                        ExhibitsTrait.Add('traitReference', 'is.partition.format.JSON');
                        ExhibitsTraits.Add(ExhibitsTrait);
                        DataPartitionPattern.Add('exhibitsTraits', ExhibitsTraits);
                    end;
                "RSMUS Data Export Format"::Parquet:
                    begin
    ...
    begin
        ...
        foreach FieldId in FieldIdList do begin
            ...
            Result.Add(
                CreateAttributeJson(
                    rsmusUtil.GetDataLakeCompliantFieldName(FldRef.Name, FldRef.Number),
                    DataFormat,
                    FldRef.Name,
                    AppliedTraits));
        end;
        if rsmusUtil.IsTablePerCompany(TableID) then begin
    ...
    begin
        ...
        foreach OldAttribute in OldAttributes do begin
            ...
            OldAttributeName := rsmusUtil.GetTextValueForKeyInJson(OldAttribute.AsObject(), 'name');

            foreach NewAttribute in NewAttributes do
                if rsmusUtil.GetTextValueForKeyInJson(NewAttribute.AsObject(), 'name') = OldAttributeName then begin
                    OldAttributeFound := true;
                    OldDataType := rsmusUtil.GetTextValueForKeyInJson(OldAttribute.AsObject(), 'dataFormat');
                    NewDataType := rsmusUtil.GetTextValueForKeyInJson(NewAttribute.AsObject(), 'dataFormat');
    ...
    var
        ...
        Value2: Text;
    begin
        Value1 := rsmusUtil.GetTextValueForKeyInJson(Attribute1.AsObject(), FieldName);
        Value2 := rsmusUtil.GetTextValueForKeyInJson(Attribute2.AsObject(), FieldName);
