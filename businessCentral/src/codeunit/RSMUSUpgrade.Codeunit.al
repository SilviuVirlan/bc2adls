...
codeunit 82672 "RSMUS Upgrade"
{
    ...
    var
        rsmusInstaller: Codeunit "RSMUS Installer";
        rsmusExecution: Codeunit "RSMUS Execution";
        InvalidFieldsMap: Dictionary of [Integer, List of [Text]];
    begin
        InvalidFieldsMap := rsmusInstaller.ListInvalidFieldsBeingExported();
        if InvalidFieldsMap.Count() > 0 then begin
            rsmusExecution.Log('ADLSE-30',
                'Upgrade preconditions not met as there are invalid fields enabled for export. Please see previous telemetry.', Verbosity::Error, DataClassification::ToBeClassified);
    ...
    begin
        if UpgradeTag.HasUpgradeTag(GetRetenPolLogEntryAddedUpgradeTag()) then
            exit;
        rsmusInstaller.AddAllowedTables();
    ...
    begin
        foreach TableID in TableIDFieldNameList.Keys() do
            Result += StrSubstNo(TableFieldsTok, rsmusUtil.GetTableCaption(TableID), rsmusUtil.Concatenate(TableIDFieldNameList.Get(TableID)));
    end;
