...
codeunit 82664 "RSMUS Util"
{
    ...
    begin
        ...
        if AddHeaders then begin
            ...
            if IsTablePerCompany(Rec.Number) then
                Payload.Append(StrSubstNo(CommaPrefixedTok, rsmusCDMUtil.GetCompanyFieldName()));
            Payload.AppendLine();
        end;
    ...
    procedure CreateFakeRecordForDeletedAction(rsmusDeletedRecord: Record "RSMUS Deleted Record"; var Rec: RecordRef)
    var
    ...
    begin
        ...
        TimestampField.Value(rsmusDeletedRecord."Deletion Timestamp");
        SystemIdField := Rec.Field(Rec.SystemIdNo());
        SystemIdField.Value(rsmusDeletedRecord."System ID");
    ...
    procedure CreateJsonPayload(RecRef: RecordRef; FieldIdList: List of [Integer]; CompanyID: Guid; ChangeType: Enum "RSMUS Change Type"; var RecordSize: Integer) RecordPayload: JsonObject
    var
        FieldRef: FieldRef;
        FieldID: Integer;
        FieldTextName: Text;
    ...
    begin
        foreach FieldID in FieldIdList do begin
            ...
            FieldTextName := GetDataLakeCompliantFieldName(FieldRef.Name, FieldRef.Number);
            FieldTextValue := ConvertFieldToText(FieldRef);
            Payload.Add(FieldTextName, FieldTextValue);
            RecordSize := RecordSize + StrLen(FieldTextName) + StrLen(FieldTextValue) + 7;
        end;
        if IsTablePerCompany(RecRef.Number) then begin
            Payload.Add('CompanyID', Format(CompanyID));
            RecordSize := RecordSize + 11 + StrLen(Format(CompanyID)) + 2;
        end;
        Payload.Add('ChangeType', Format(ChangeType));
        RecordSize := RecordSize + 12 + StrLen(Format(ChangeType)) + 2;
