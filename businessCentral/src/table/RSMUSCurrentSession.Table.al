...
table 82665 "RSMUS Current Session"
{
    Access = Internal;
    ...
    procedure Start(rsmusTableID: Integer)
    var
        rsmusUtil: Codeunit "RSMUS Util";
    begin
        Rec.Init();
        Rec."Table ID" := rsmusTableID;
        ...
        if not Rec.Insert() then
            Error(InsertFailedErr, rsmusUtil.GetTableCaption(rsmusTableID));
    end;

    procedure Stop(rsmusTableID: Integer; EmitTelemetry: Boolean)
    var
        rsmusExecution: Codeunit "RSMUS Execution";
        rsmusUtil: Codeunit "RSMUS Util";
    begin
        if not Rec.Get(rsmusTableID, CompanyName()) then
            exit;
        if not Rec.Delete() then
            rsmusExecution.Log('ADLSE-036', StrSubstNo(CouldNotStopSessionErr, Rec."Session ID", rsmusUtil.GetTableCaption(rsmusTableID), CompanyName()), Verbosity::Error, DataClassification::ToBeClassified);
    end;
        ...
        if Rec.FindSet(false) then
            repeat
                if IsSessionActive() then
                    Session.StopSession(Rec."Session ID", StrSubstNo(SessionTerminatedMsg, rsmusUtil.GetTableCaption(Rec."Table ID")));
            until Rec.Next() = 0;
